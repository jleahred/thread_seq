...
private:
  ...
  std::map<std::thread::id, int>
      map_sync_waitting; //  count by thread_id sync queued
...
ThreadSeq::ThreadSeq(cfg c) : impl(new Impl(c)) {}
ThreadSeq::~ThreadSeq() {}

void ThreadSeq::stop() { impl->stop(); }
...
void ThreadSeq::run_sync(std::function<void(void)> fn) {
...

  {
    ...
    impl->m_lambdas.lock();
    impl->lambda_sync = impl->lambda_sync =
        std::make_shared<std::function<void(void)>>(fn);
...

  {
    ...
    while (!impl->cv_sync_finished.wait_for(lk, 10ms, [this] {
      ...
      const auto stop_waitting =
          !this->impl->lambda_sync || !this->impl->keep_running;
  ...
  if (impl->keep_running && impl->lambda_sync) {
    impl->config.on_critical_error(CriticalError::InternalError);
...
void ThreadSeq::run_async(std::function<void(void)> fn) {
...

  {
    ...
    while (impl->keep_running) {
      impl->m_lambdas.lock();
      if (impl->lambda_async.size() < impl->config.max_sync_queue_size &&
          impl->keep_running) {
        impl->m_lambdas.unlock();
        break;
      }
  ...
  while (ts->keep_running) {
    {
      ...
      std::unique_lock<std::mutex> lk_process_pendings(
          ts->m_cv_process_pendings);
      while (
          !ts->cv_process_pendings.wait_for(lk_process_pendings, 100ms, [&ts] {
            //                std::lock_guard(ts->m_lambdas);    //  c++20
            std::lock_guard<std::mutex> lock_lambdas(ts->m_lambdas);
            const auto stop_waitting = ts->lambda_sync.get() ||
                                       ts->lambda_async.size() ||
                                       !ts->keep_running;
            return stop_waitting;
          })) {
        if (!ts->keep_running)
      ...
      if (ts->keep_running && ts->lambda_sync) {
        ...
        ts->m_lambdas.unlock();
        {
          try {
            ...
            ts->config.on_critical_error(CriticalError::NotExpectedException);
          }
        }
        ts->m_lambdas.lock();
...
std::thread::id DeadLockDetector::last_locked_threadid() {
...
void DeadLockDetector::register_sync_call(std::thread::id caller_thread_id) {
  ...
  if (map_sync_waitting[caller_thread_id] != 0 &&
      caller_thread_id != last_locked_threadid()) {
    m.unlock();
...
void DeadLockDetector::unregister_sync_call(std::thread::id caller_thread_id) {
  m.lock();
  if (map_sync_waitting[caller_thread_id] > 0) {
    map_sync_waitting[caller_thread_id] =
        map_sync_waitting[caller_thread_id] - 1;
    if (map_sync_waitting[caller_thread_id] == 0) {
...
cfg::cfg()
    : max_sync_queue_size(DEFAULT_MAX_QUEUE_SIZE),
      ...
      on_critical_error([](CriticalError) {}) {}
...
cfg cfg::set_on_critical_error(
    std::function<void(CriticalError err)> callback) {
  this->on_critical_error = callback;
  return *this;
}
